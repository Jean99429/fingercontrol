...
状态：实现规格 v3.1（2026-09-08 UI 统一版）
项目路径：`/Users/jean/Documents/ChatGPT/Fingercontrol 2`
...
- 首页以两只镜像手掌作为主要界面，八根非拇指直接承载可编辑文字。
- 所有手指循环使用 `af_heart`、`am_puck`、`af_sarah` 三种音色朗读。
- 识别到手后在对应指尖常显文字，捏合时立即发音。
...
- 只有选择 CAMERA 并点击 `START PERFORMING` 后才请求权限。
- 允许选择浏览器可用的摄像头设备。
...
- 深色全屏背景。
- 两只张开的手是页面视觉与操作核心。
- 两只手共用同一份右手 SVG，左手通过水平镜像生成。
- 顶部标题为 `Pinch a finger. Make it speak.`，下方直接解释分配文字与拇指触发方式。
- 右上角提供紧凑的 `CAMERA / VIDEO` 分段切换。
- 中部左右两只手等宽、放大展示。
- 左手必须在页面左侧，右手必须在页面右侧。
- 底部使用紧凑来源控制条；CAMERA 主按钮为 `START PERFORMING`，UPLOAD VIDEO 为 `ANALYZE VIDEO`。
...
- 八根非拇指各有一个透明文字输入，水平居中在对应手指上。
- 输入无外框、无下划线、无暗色底、无旋转角度。
- 每个槽使用独立字体、字重、斜体和颜色，具体映射见第 12 节。
- 输入聚焦时可直接替换文字；语音预览按钮仅在 hover / focus 时出现。
- 不显示手指映射下拉框、声音参数或效果设置。
- 拇指不显示文字，只显示 `#E60340` 触发点。
...
- MediaPipe 检测到手后，食指、中指、无名指和小指文字始终显示在各自指尖附近。
- 常显文字不依赖 `TRACKING` 开关，也不依赖 ACTIVE 捏合状态。
- 表演页直接读取设置页八个槽的文字和共享 `SLOT_VISUALS`，保证字体、字重、斜体与颜色一致。
- 文字无底色、无描边、无下划线和旋转。
- 字号以视频短边为基准响应式缩放；长文字自动缩小，避免越出画布。
- ACTIVE 状态可轻微增大对应文字，但不改变其字体和颜色。
- 捏合只触发分配音色的语音；单次捏合不重复播放。
...
- 次要文字：`#92949A`。
- UI 主强调色：薄荷绿 `#5CFFB0`。
- 手指追踪点：严格使用信号红 `#E60340`，首页拇指点与内页指尖点一致。
- 红色不得用于普通按钮或大面积状态；错误提示可使用琥珀色。
- 圆角适中、细边框、低对比表面，不使用游戏 HUD 和过度阴影。
...
- 左右手并排、等宽，左手由右手素材镜像生成。
- 每只手四个透明文字输入分别居中于 INDEX、MIDDLE、RING、PINKY。
- 输入无外框、无下划线、无暗色底和旋转。
